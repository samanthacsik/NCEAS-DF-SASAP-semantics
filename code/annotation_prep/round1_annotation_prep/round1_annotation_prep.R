...
round1_pkgs <- read_csv(here::here("data", "queries", "query2020-10-09", "unique_pkgs.csv")) %>% 
  ...
  rowid_to_column("ID") %>% 
  mutate(ID = str_pad(string = ID, width = 3, side = "left", pad = 0))

round1_pkgs_short <- round1_pkgs %>% select(identifier, ID)
...
round1_test <- full_join(round1, round1_pkgs)
...
add_ID_spp <- full_join(round1_species, round1_pkgs_short) %>% 
  drop_na(entityName)

round1_species <- add_ID_spp
...
add_ID_lengths <- full_join(round1_fishLengths, round1_pkgs_short) %>% 
  drop_na(entityName)

round1_fishLengths <- add_ID_lengths
...
rm(round1_attributes_all, round1_attributes_prevSort, round1_test, round1_pkgs, round1_pkgs_short, round1_pkgIDS, add_ID_spp, add_ID_lengths, round1_fishLengths)
