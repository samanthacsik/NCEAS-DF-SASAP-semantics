...
annotate_multiple_dataTables_multiple_attributes <- function(doc, entity_num, eml_att_num, current_attribute_id, attributeName_subset){
  ...
  # doc$dataset$dataTable[[entity_num]]$attributeList$attribute[[eml_att_num]]$annotation$propertyURI <- list(label = "contains measurements of",
  ...
  doc$dataset$dataTable[[entity_num]]$attributeList$attribute[[eml_att_num]]$annotation$propertyURI <- list(label = current_propertyLabel,
