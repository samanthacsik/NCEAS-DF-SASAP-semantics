# title: batch update of datapackages with semantic annotations -- SALMON DATA INTEGRATION TEST PORTAL 004
# author: "Sam Csik"
...
attributes <- SDI_atts_004 %>% 
  filter(!attributeName %in% c("species_common_name", "length_froz", "length_wet"))
# -----------------------------------------------
...
# -------------------------------------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------------------------------------
# ANNOTATE BY HAND ----- NEED TO UPDATE SO THAT THIS IS A DATATABLE, NOT OTHERENTITY 
# -------------------------------------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------------------------------------
# -------------------------------------------------------------------------------------------------------------------------
...
##############################
# dataTable 1 (dataset_3106_salmonbiodata__v1.csv) 
##############################

# attribute 11, species_common_name = Chinook, Chum, Coho, Sockeye, Pink
doc$dataset$dataTable$attributeList$attribute[[11]]$id <- "dataTable1_spp"
doc$dataset$dataTable$attributeList$attribute[[11]]$annotation <- list(
...
# attribute 12/13, length_froz, length_wet = Fork length
doc$dataset$dataTable$attributeList$attribute[[12]]$id <- "dataTable1_length"
doc$dataset$dataTable$attributeList$attribute[[12]]$annotation <-  list(
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
...
# >>>>>>>> UPDATE HERE BEFORE EACH RUN <<<<<<<<<< 
 # write_csv(old_new_PIDs, here::here("data", "updated_pkgs", "SDI_test_portal", "SDI_004.csv"))
# ------------------------------------------------


# old metadata pid: urn:uuid:77bf16c1-2120-466f-8f5e-f5694d56870a
# new metadata pid: urn:uuid:b8a72069-1d41-4409-a3d1-7a43dfbbd58a
# old rm: resource_map_urn:uuid:77bf16c1-2120-466f-8f5e-f5694d56870a
# new rm: resource_map_urn:uuid:b8a72069-1d41-4409-a3d1-7a43dfbbd58a
...
#------------------------------------------------------------------------------------------------------
# add 'Fork length' annotation to 'length_wet
#------------------------------------------------------------------------------------------------------
...
pkg <- get_package(devnceas@mn, 
                   "urn:uuid:b8a72069-1d41-4409-a3d1-7a43dfbbd58a", # this is actually the metadata pid from solr (will throw a warning but that's okay)
                   file_names = TRUE)
...
#------------------------------
# dataTable 1 (length_wet) 
#------------------------------

# attribute 12/13, length_froz, length_wet = Fork length
doc$dataset$dataTable$attributeList$attribute[[13]]$id <- "dataTable1_length2"
doc$dataset$dataTable$attributeList$attribute[[13]]$annotation <-  list(
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
       valueURI = list(label = "Fork length", valueURI = "http://purl.dataone.org/odo/salmon_000128"))
)
...
# old metadata pid: urn:uuid:b8a72069-1d41-4409-a3d1-7a43dfbbd58a
# new metadata pid: urn:uuid:2390d855-13fc-4d1f-9757-49e278f320d6
