# Alaska Department of Fish and Game, Division of Commercial Fisheries, Central Region. 2018. Salmon age, sex, and length data from the Copper and Coghill Rivers, Alaska, 1965-2016. Knowledge Network for Biocomplexity.

# title: batch update of datapackages with semantic annotations -- ROUND 1, 020 (Salmon ASL Copper and Coghill Rivers)
# author: "Sam Csik"
...
#-----------------------------
# dataTable 1 (ASL_formatted_CoghillWeir.csv)
#-----------------------------

# attribute 5, Species = chinook, sockeye ?
doc$dataset$dataTable[[1]]$attributeList$attribute[[5]]$id <- "dataTable1_spp"
doc$dataset$dataTable[[1]]$attributeList$attribute[[5]]$annotation <- list(
...
# attribute 11, Length = mid-eye to fork of tail
doc$dataset$dataTable[[1]]$attributeList$attribute[[11]]$id <- "dataTable1_length"
doc$dataset$dataTable[[1]]$attributeList$attribute[[11]]$annotation <- list(
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
...
#-----------------------------
# dataTable 2 (ASL_formatted_CopperRiverSockeye.csv)
#-----------------------------

# attribute 5, Species = sockeye ?
...
# attribute 8, Length = mid-eye to fork of tail
doc$dataset$dataTable[[2]]$attributeList$attribute[[8]]$id <- "dataTable2_length"
doc$dataset$dataTable[[2]]$attributeList$attribute[[8]]$annotation <- list(
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
       valueURI = list(label = "Mid-orbit to fork of tail length", valueURI = "http://purl.dataone.org/odo/salmon_000133"))
)
...
write_csv(old_new_PIDs, here::here("data", "updated_pkgs", "round1", "round1_020.csv"))
# ------------------------------------------------


# old metadata pid: doi:10.5063/F1DB8030
# new metadata pid: doi:10.5063/WH2NDP
# old rm: resource_map_doi:10.5063/F1DB8030
# new rm: resource_map_doi:10.5063/WH2NDP