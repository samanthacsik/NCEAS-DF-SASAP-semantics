# Alaska Department of Fish and Game, Mark, Tag, and Age Lab. Sockeye salmon age measurements from scale data, Alaska, 2011-2017. Knowledge Network for Biocomplexity.

# title: batch update of datapackages with semantic annotations -- ROUND 1, 018 (Sockeye age measurements from scale data)
# author: "Sam Csik"
...
attributes <- round1_018 %>% 
  filter(!attributeName %in% c("length")) %>% 
...
#-----------------------------
# dataTable 1 (sockeye_scale_data.csv) 
#-----------------------------
...
# attribute 7, length = Fish length measurement type (NOTE: no specific length measurement type given in dataset or abstract)
...
doc$dataset$dataTable$attributeList$attribute[[7]]$annotation <- list(
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
       valueURI = list(label = "Fish length measurement type", valueURI = "http://purl.dataone.org/odo/salmon_000127"))
)
...
write_csv(old_new_PIDs, here::here("data", "updated_pkgs", "round1", "round1_018.csv"))
# ------------------------------------------------


# old metadata pid: doi:10.5063/F1ZW1J77
# new metadata pid: doi:10.5063/0863Q4
# old rm: resource_map_urn:uuid:446a2071-2a7d-4950-82fc-100823636be1
# new rm: resource_map_doi:10.5063/0863Q4
...
old_new_PIDs <- data.frame(
  old_metadataPID = "doi:10.5063/0863Q4",
  old_resource_map = "doi:10.5063/VM49PM",
  new_metadataPID = "resource_map_doi:10.5063/VM49PM",
  new_resource_map = "resource_map_doi:10.5063/0863Q4"
)
