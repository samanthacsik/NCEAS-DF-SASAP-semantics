# Alaska Department of Fish and Game, Division of Sport Fish. 2018. Daily salmon escapement counts from in-season online reporting by the Alaska Department of Fish and Game, Alaska, 1955-2016. Knowledge Network for Biocomplexity.

# THIS DATA PACKAGE REQUIRED SOME FURTHER WRANGLING BEFORE UPDATE WAS POSSIBLE (SEE STEP 0)

# title: batch update of datapackages with semantic annotations -- ROUND 1, 024 (Daily escapement from in-season online reporting by ADF&G)
# author: "Sam Csik"
...
#########################################################################################
# STEP 0: some data wrangling to get Species attributes in order....265 datasets, each with a different species
#########################################################################################

single_spp <- round1_024 %>% 
  filter(attributeName == "Species")

others <- single_spp %>% filter(entityName %in% c("SportFish.csv", "Yukon Pilot Station Other 95-16.csv"))

chinook <- single_spp %>% 
  filter(str_detect(entityName, "(?i)chinook")) %>% 
  select(-assigned_valueURI, -prefName, -grouping) %>% 
  mutate(assigned_valueURI = rep("http://purl.dataone.org/odo/salmon_000239")) %>% 
  mutate(prefName = rep("Chinook salmon")) %>% 
  mutate(grouping = rep("specific_spp"))

chum <- single_spp %>% 
  filter(str_detect(entityName, "(?i)chum")) %>% 
  select(-assigned_valueURI, -prefName, -grouping) %>% 
  mutate(assigned_valueURI = rep("http://purl.dataone.org/odo/salmon_000240")) %>% 
  mutate(prefName = rep("Chum salmon")) %>% 
  mutate(grouping = rep("specific_spp"))

coho <- single_spp %>% 
  filter(str_detect(entityName, "(?i)coho")) %>% 
  select(-assigned_valueURI, -prefName, -grouping) %>% 
  mutate(assigned_valueURI = rep("http://purl.dataone.org/odo/salmon_000243")) %>% 
  mutate(prefName = rep("Coho salmon")) %>% 
  mutate(grouping = rep("specific_spp"))

pink <- single_spp %>% 
  filter(str_detect(entityName, "(?i)pink")) %>% 
  select(-assigned_valueURI, -prefName, -grouping) %>% 
  mutate(assigned_valueURI = rep("http://purl.dataone.org/odo/salmon_000241")) %>% 
  mutate(prefName = rep("Pink salmon")) %>% 
  mutate(grouping = rep("specific_spp"))

sockeye <- single_spp %>% 
  filter(str_detect(entityName, "(?i)sockeye") |
         entityName == "Olga SockeyeER 08.csv") %>% 
  select(-assigned_valueURI, -prefName, -grouping) %>% 
  mutate(assigned_valueURI = rep("http://purl.dataone.org/odo/salmon_000242")) %>% 
  mutate(prefName = rep("Sockeye salmon")) %>% 
  mutate(grouping = rep("specific_spp"))

steelhead <- single_spp %>% 
  filter(str_detect(entityName, "(?i)steelhead")) %>% 
  select(-assigned_valueURI, -prefName, -grouping) %>% 
  mutate(assigned_valueURI = rep("http://purl.dataone.org/odo/salmon_000570")) %>% 
  mutate(prefName = rep("Steelhead trout")) %>% 
  mutate(grouping = rep("specific_spp"))

cutthroat <-  single_spp %>% 
  filter(str_detect(entityName, "(?i)cutthroat")) %>% 
  select(-assigned_valueURI, -prefName, -grouping) %>% 
  mutate(assigned_valueURI = rep("http://purl.dataone.org/odo/salmon_000582")) %>% 
  mutate(prefName = rep("Cutthroat trout")) %>% 
  mutate(grouping = rep("specific_spp"))

dollyvarden <-  single_spp %>% 
  filter(str_detect(entityName, "(?i)dolly")) %>% 
  select(-assigned_valueURI, -prefName, -grouping) %>% 
  mutate(assigned_valueURI = rep("http://purl.dataone.org/odo/salmon_000569")) %>% 
  mutate(prefName = rep("Dolly Varden trout")) %>% 
  mutate(grouping = rep("specific_spp"))

all_spp <- rbind(others, chinook, chum, coho, pink, sockeye, steelhead, cutthroat, dollyvarden)

test <- round1_024 %>% 
  filter(attributeName != "Species")

round1_024_new <- rbind(test, all_spp)
...
attributes <- round1_024_new %>% 
  filter(!entityName %in% c("SportFish.csv") |
...
#-----------------------------
# dataTable 1 (SportFish.csv)
#-----------------------------

# attribute 3, species = sockeye, chum, coho, chinook
doc$dataset$dataTable[[1]]$attributeList$attribute[[3]]$id <- "dataTable1_spp"
doc$dataset$dataTable[[1]]$attributeList$attribute[[3]]$annotation <- list(
  ...
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
       valueURI = list(label = "Cutthroat trout", valueURI = "http://purl.dataone.org/odo/salmon_000582")),
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
...
write_csv(old_new_PIDs, here::here("data", "updated_pkgs", "round1", "round1_024.csv"))
# ------------------------------------------------


# old metadata pid: doi:10.5063/F1GB22B9
# new metadata pid: doi:10.5063/GH9GBN
# old rm: resource_map_doi:10.5063/F1GB22B9
# new rm: resource_map_doi:10.5063/GH9GBN