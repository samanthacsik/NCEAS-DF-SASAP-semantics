# Alaska Department of Fish and Game, Division of Commercial Fisheries. Salmon age, sex, and length data from Westward and Southeast Alaska, 1979-2017. Knowledge Network for Biocomplexity.

# title: batch update of datapackages with semantic annotations -- ROUND 1, 036 (ASL from Westward and Southeast AK)
# author: "Sam Csik"
...
attributes <- round1_036 %>% 
  filter(!attributeName %in% c("Species", "SPECIES", "Length", "LENGTH_MM")) %>% 
...
#-----------------------------
# dataTable 1 (ASL_formatted_SoutheastKodiak.csv) 
#-----------------------------

# attribute 10, Species = chinook, chum, coho, pink, sockeye
doc$dataset$dataTable[[1]]$attributeList$attribute[[10]]$id <- "dataTable1_species"
doc$dataset$dataTable[[1]]$attributeList$attribute[[10]]$annotation <- list(
...
# attribute 13, Length = mid-orbit to fork of tail length
doc$dataset$dataTable[[1]]$attributeList$attribute[[13]]$id <- "dataTable1_length"
doc$dataset$dataTable[[1]]$attributeList$attribute[[13]]$annotation <- list(
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
       valueURI = list(label = "Mid-orbit to fork of tail length", valueURI = "http://purl.dataone.org/odo/salmon_000133"))
)

#-----------------------------
# dataTable 2 (ASL DATA EXPORT.csv) 
#-----------------------------

# attribute 14, Species = chinook, chum, coho, pink, sockeye
doc$dataset$dataTable[[2]]$attributeList$attribute[[14]]$id <- "dataTable2_species"
doc$dataset$dataTable[[2]]$attributeList$attribute[[14]]$annotation <- list(
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
       valueURI = list(label = "Chum salmon", valueURI = "http://purl.dataone.org/odo/salmon_000240")),
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
       valueURI = list(label = "Sockeye salmon", valueURI = "http://purl.dataone.org/odo/salmon_000242")),
...
# attribute 17, Length = mid-orbit to fork of tail length
doc$dataset$dataTable[[2]]$attributeList$attribute[[17]]$id <- "dataTable2_length"
doc$dataset$dataTable[[2]]$attributeList$attribute[[17]]$annotation <- list(
  list(propertyURI = list(label = "contains measurements of type", propertyURI = containsMeasurementsofType),
       valueURI = list(label = "Mid-orbit to fork of tail length", valueURI = "http://purl.dataone.org/odo/salmon_000133"))
)
...
# old metadata pid: doi:10.5063/F1SJ1HWB
